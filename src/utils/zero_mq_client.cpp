...
void IpcPath_check(std::string ipc_path) {
    // 确保 IPC 路径存在且可写入
    std::filesystem::path socket_dir = std::filesystem::path(ipc_path).parent_path();
    if (!socket_dir.empty() && !std::filesystem::exists(socket_dir)) {
        std::filesystem::create_directories(socket_dir);
        std::cout << "ZeroMqPublisher 创建ipc目录: " << socket_dir << std::endl;
    }

    //如果该套接字文件已存在，先删除它
    if (std::filesystem::exists(ipc_path)) {
        std::filesystem::remove(ipc_path);
        std::cout << "ZeroMqPublisher 删除旧的套接字文件: " << ipc_path << std::endl;
    }
}
...
void ZeroMqSubscriber::setMessageHandler(MessageHandler handler, void *handler_param) {
    ...
    poller_->addSocket(operation_, [handler](void *param, zmq::message_t &message, std::string &topic) {
        handler(param, message, topic);
    }, handler_param, topic_);
}
